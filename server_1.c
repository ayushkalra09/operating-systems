...
//   printf("enter start node");
//   scanf("%d", &stnode);
//   thread_data res = {gr.adj_mat, gr.n, stnode - 1, -1};
//   pthread_create(&tid, NULL, bfs_1, &res);
//   pthread_join(tid, NULL);
//   // printf("DFS = %s\n", res.res);
//   strcpy(req.graph_file_name, res.res);
//   struct message ret_msg = {r->mtype, r->mtype, req};
...
int main()
{
  ...
  while (1)
  {
    ...
    if (msgrcv(server_queue, &msg, sizeof(msg), SERVER_1_PORT, 0) == -1)
    {
      perror("msgrcv");
      exit(1);
    }
